...
async function sqlite_json_insert(json_dict,table_name) {
    ...
    try {
        ...
        let sql = "INSERT INTO "+table_name+"("+table_keys+") VALUES ('" + table_values+"')";
        console.log(sql);
...
async function check_login_and_load_certificates(user_name,user_password) {
    ...
    try {
        console.log("1");
        let db_con = await sqlite3_async.open(db_name);
        let let_sql_query = 'SELECT * FROM User where User.User_Name = "'+user_name+'" ;';
        ...
        if (result.length > 0) {
            ...
            if (user_info['User_Password'] === user_password) {
                ...
                console.log("enrollment_signingIdentity = "+ enrollment_signingIdentity);
                console.log("3");
