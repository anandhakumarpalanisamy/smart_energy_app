...
function create_user_asset_in_chaincode(){
    ...
    var user_json  = {
        "User_Name":document.getElementById("User_Name").value,
        "User_Password":document.getElementById("User_Password").value,
        "User_Smart_Meter_Id":document.getElementById("User_House_Number").value,
    };
    document.getElementById("register_loader").style.display = "block";
    $.ajax({
            type: "POST",
...
$("#buy_form").submit(function(e) {
    ...
    $.ajax({
            ...
            complete: function(data)
                {
                    document.getElementById("buy_power_confirm_loader").style.display = "none";
                    var json = JSON.parse(data.responseText.replace(/\bNaN\b/g, "null"));
                    alert(json['status']);
                    if(json['status'] == "success") {
                        //$(".session_user_name").text(json['user_name']);
...
$("#ad_submit").click(function() {
    ...
    var user_json  = {
        ...
        "Energy_To_Sell":document.getElementById("energy_selling").value, 
...
Energy_Sources = '<span><label><i class="fas fa-sun"></i> Solar ,</label>&nbsp;&nbsp;<label><i class="fas fa-fan"></i> Wind</label></span>';
...
var add_content = ' <div class="w3-row w3-padding-24 w3-animate-bottom">'
            ...
            + '<span class="w3-right w3-opacity ad_time_stamp"><i class="far fa-clock" aria-hidden="true"></i> '+timestamp+'</span>'
            + '<h4 class="ad_user_name">'+user_name+'</h4><br>'
            +'<div class="ad_user_id" style="display:none;">'+user_id+'</div>'
            + '<hr class="w3-clear">'
            ...
            + '<p >'
            + '<span class="w3-text-dark-gray"><i class="fas fa-coins" aria-hidden="true"></i> Price (Energy Tokens)</span><br>'
            + '<span class="ad_cost">'+cost+'</span> / kwh'
            + '</p>'
            + '</div>'
            ...
            + '<span class="ad_energy_to_sell">'+Energy_To_Sell+' kwh</span>'
            + '</p>'
            + '</div>'
            ...
            + '<span class="ad_energy_sources">'+Energy_Sources+' </span>'
            + '</p>'
            + '</div>'
            ...
            + '<span class="ad_capacity">'+Capacity+' kwh  </span>'
            + '</p>'
            + '</div>'
            ...
            + '<button  type="button" onclick="buy_button_clicked.call(this)" class="buy_energy_button w3-button w3-theme-d2 w3-margin-bottom w3-right w3-teal w3-hover-opacity"><i class="fa fa-shopping-cart"></i> Buy</button>'
            + '</div>'
            + '</div>'
            ...
            + '<span class="ad_seller_rating">'+seller_rating+'%  <i class="far fa-heart" aria-hidden="true"></i></span>'
            + '</p>'
            ...
            + '<span class="ad_price_comparison">'+price_comparison+'% <i class="far fa-arrow-alt-circle-down" aria-hidden="true"></i> </span>'
            + '</p>'
            + '</div>'
            + '</div>';
    return add_content;
        ...
        complete: function(data)
            {
                ...
                if (json["status"] ==="success") {
                    ...
                    for (i = 0; i < json["data"].length; i++) {
                        ...
                        joined_ad_content = joined_ad_content + template_ad_content(json["data"][i]);
